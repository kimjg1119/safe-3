...
object WebServer extends {
  ...
  def run(port: Int): Unit = {
    ...
    val handlerFlow = {
      ...
      def chatInSink(uid: String) =
        Sink.actorRef[Event](cmdActor, ParticipantLeft(uid))
      ...
      (uid: String) => {
        ...
        val out =
          Source
            .actorRef[Protocol.Message](1, OverflowStrategy.fail)
    ...
    implicit def myExceptionHandler: ExceptionHandler =
      ExceptionHandler { case _: NoFileSelectedException =>
        extractUri { uri =>
    ...
    def websocketFlow(uid: String): Flow[Message, Message, Any] =
      Flow[Message]
        .collect { case TextMessage.Strict(msg) =>
          ...
          extractRequest { req =>
            {
              req.header[UpgradeToWebSocket] match {
                case Some(upgrade) =>
                  complete(upgrade.handleMessages(websocketFlow(uid)))
                case None =>
                  complete(
                    StatusCodes.BadRequest,
                    HttpEntity("Not a valid websocket request!")
                  )
              }
            }
          }
    ...
    println(
      s"Open browser and navigate http://localhost:$port/\nPress RETURN to stop..."
    )
