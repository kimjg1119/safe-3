...
object JSFromHTML {
  ...
  def parseScripts(fileName: String): Try[(Stmts, ExcLog)] = {
    ...
    val filtered =
      toList(source.getAllElements(HTMLElementName.SCRIPT)).filter(x => {
    ...
    val codeContents2: List[(String, (Int, Int), String)] =
      if (!loadingDir.exists) codeContents
        ...
        loadingDir.listFiles.foldLeft(codeContents)((li, ff) => {
          ...
          val name = ff.getName
          val ss =
            ("#loading#" + name, bogus, source.mkString.replace("\r", ""))
          source.close
          li :+ ss
        })
      }
  ...
  private def externalSource(
    ...
    if (srcName.length > 0) {
      ...
      val external = new File(
        "downloads/" + srcName.substring(srcName.lastIndexOf('/') + 1)
      )
      ...
      if (pathf.exists) {
        ...
        val result =
          Some(path.toString, bogus, source.mkString.replace("\r", ""))
        source.close
        result
      } else {
